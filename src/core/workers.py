                    ...
                    try:
                        def format_date(date_str):
                            ...
                            try:
                                ...
                                strftime_format = self.manga_custom_date_format
                                strftime_format = strftime_format.replace("YYYY", "%Y").replace("MM", "%m").replace("DD", "%d")
                        ...
                        format_values = {
                            ...
                            'user': str(self.post.get('user', '')),
                            'service': str(self.post.get('service', '')),
                            'title': str(self.post.get('title', '')),
                            'name': original_filename_cleaned_base,
                            
                            # Apply the fallback logic: use 'published_date' if the desired date is missing
    ...
    def process(self):
        ...
        try:
                ...
                if self.use_date_prefix_for_subfolder:
                    ...
                    if published_date_str:
                        try:
                            ...
                            if "{post}" in self.date_prefix_format:
                                final_subfolder_name = final_subfolder_name.replace("{post}", original_cleaned_post_title_for_sub)
                            else:
                                final_subfolder_name = f"{final_subfolder_name} {original_cleaned_post_title_for_sub}"
                            ...
                            self.logger(f"   ⚠️ Could not parse date '{published_date_str}' or apply custom format. Using default prefix. Error: {e}")
            ...
            if (self.show_external_links or self.extract_links_only) and post_content_html:
                ...
                try:
                    ...
                    for link_url, link_text in unique_links_data.items():
                        ...
                        # Combine the URL and key right here if a key was found separately
                        ...
                        if platform not in scraped_platforms:
                            # Emit the complete URL and an empty key, as it's now part of the URL
            ...
            if self.archive_only_mode and not self.extract_links_only:
                # Check if there's an archive and also other files
                ...
                if has_archive and len(all_files_from_post_api) > 1:
                    ...
                    # Filter the list to keep ONLY archives
